...
async def run_evaluation():
    ...
    for i, item in enumerate(dataset):
        ...
        # Retrieve the full state to check agents
        ...
        if not retrieved_contexts:
            data_ctx = full_state.values.get("data_context", {})
            if data_ctx:
                ...
                retrieved_contexts.append("No context retrieved (direct LLM knowledge).")
        ...
        results.append({
            ...
            "actual_agent": full_state.values.get("active_agent"),
    ...
    print(f"Detailed results saved to tests/eval_results.csv")
    
    # --- Adjusted Metrics (excluding structurally unfair questions) ---
    df_faithful = df_results[df_results["skip_faithfulness"] == False]
    ...
    print(f"Faithfulness (adjusted, {len(df_faithful)} questions): {adj_faithfulness:.4f}")
    print(f"Answer Relevancy (adjusted, {len(df_relevant)} questions): {adj_relevancy:.4f}")
    print(f"")
    print(f"Excluded from faithfulness: {len(df_results) - len(df_faithful)} questions (skip_faithfulness=True)")
    print(f"Excluded from relevancy: {len(df_results) - len(df_relevant)} questions (score=0.0, guard-rail rejections)")
